...
output_dir <- "Model_Output/Extra/THgLOI/"
model_dir <- "Saved_Models/Extra/THgLOI/"
fig_dir <- "Figures/Extra/THgLOI/"
...
Test_Errors <- Test_Dat  %>% summarize(
  ...
  Bias=mean(Pred-log10THgLOI)) 
#         MAE      RMSE        Bias
#   0.1871957 0.2337868  0.04603414
...
mantel_test$signif # r=0.07, p=.001 There is spatial autocorrelation in residuals
...
for(i in 1:length(final.preds)){
  ...
  autoplot(partial1, size=1.2) + theme_minimal() + xlab(paste0(pred_lab)) + ylab("log10THgLOI") +
    theme(text=element_text(size=20))  #+
