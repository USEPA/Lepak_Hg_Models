...
# * Rerun isotope model but just for D202 to see if predictors and accuracy change
...
output_dir <- "Model_Output/Extra/Iso_D202/"
model_dir <- "Saved_Models/Extra/Iso_D202/"
fig_dir <- "Figures/Extra/Iso_D202/"
...
p <- ncol(Train_Dat_run)-1 # 127 preds - includes LOI, THg, MeHg




# Tune mtry across isos (just D202 now)
...
for(i in 1:nrow(grid_search)){
  ...
  mvrf.i <- rfsrc(D202 ~., data = Train_Dat_run, samptype="swr", importance="none", splitrule = "mse", mtry=grid_search$mtry[i], ntree=2000, block.size=NULL, nodesize = 1)
  ...
  ### *** NEED TO CHANGE SPLITRULE TO MSE FOR UNIVARIATE RESPONSE
  ...
  grid_search$D202_OOB_error[i] <- get.mv.error(mvrf.i)[1]
  
  rm(mvrf.i)
}

write.csv(grid_search, "Tables/Grid_Search/D202_grid_srch.csv", row.names = F)
# grid_search <- read.csv("Tables/Grid_Search/D202_grid_srch.csv")
...
##### Let's go with node=1, i=4, which is floor(p/3) (kind of a mess, just choose same as fingerprint model)
...
set.seed(36)
p <- ncol(Train_Dat_run)-1 # 127 preds - includes LOI, THg, MeHg
mvrf.all <- rfsrc(D202 ~., data = Train_Dat_run, samptype="swr", importance="permute", splitrule = "mse", mtry=floor(p/3), ntree=5000, block.size=1, nodesize = 1)
...
mvrf.all
# These are just D202 performance metrics - not average
# (OOB) R squared: 0.43833206
# (OOB) Requested performance error: 0.56166794 (mse)
...
# D199      D200      D202 
# 0.5807113 0.8042107 0.7722661 (old)
# New for D202
# 0.7494451 (close to previous)
...
write.csv(imp, paste0(output_dir, "/D202_Pred_Importance_withoutRFE.csv"), row.names = FALSE)
...
for(i in 1:p){
  ...
  set.seed(13) 
  rf <- rfsrc(D202 ~., data = All_dat_preds_loop, samptype="swr", importance="permute", splitrule = "mse", mtry=max(floor(nump/3), 1), ntree=5000, block.size=1, nodesize = 1)
  ...
  # OOB error - D202
  OOB.rmse.D202[i] <- sqrt(mean((Train_Dat_run$D202-rf$predicted.oob)^2)) # OOB rmse
  OOB.mae.D202[i] <- mean(abs(Train_Dat_run$D202-rf$predicted.oob)) # oob mae
  OOB.bias.D202[i] <- mean(rf$predicted.oob-Train_Dat_run$D202) # oob bias
  # Training error - D202
  train.rmse.D202[i] <- sqrt(mean((Train_Dat_run$D202-rf$predicted)^2)) # training rmse
  train.mae.D202[i] <- mean(abs(Train_Dat_run$D202-rf$predicted)) #  training mae
  train.bias.D202[i] <- mean(rf$predicted-Train_Dat_run$D202) # training bias
...
ggplot(RFE_info)+
  geom_line(aes(x=1:length(OOB_bias_D202), y=OOB_bias_D202), col="red") +
  geom_point(aes(x=1:length(OOB_bias_D202), y=OOB_bias_D202), col="red")+
...
# Plot OOB error
RFE_info %>% filter(NumVars %in% 1:50) %>% ggplot(aes(x=NumVars, y=OOB_mae_D202, label=Worst_Var)) +
  geom_point(size=3, aes(col="D202")) +
  geom_line(size=1.2, aes(col="D202")) +
  # 
...
RFE_info %>% filter(NumVars %in% 1:50) %>% ggplot(aes(x=NumVars, y=OOB_rmse_D202, label=Worst_Var)) +
  geom_point(size=3, aes(col="D202")) +
  geom_line(size=1.2, aes(col="D202")) +
  # 
...
write.csv(RFE_info_write, paste0(output_dir, "D202_rf_RFE_info.csv"), row.names = FALSE) # R object (list) with the variable importances at each iteration of the algorithm



as.matrix(head(RFE_info_write, 12)[,1])
