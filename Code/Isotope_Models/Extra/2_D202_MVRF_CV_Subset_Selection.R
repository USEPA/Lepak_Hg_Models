...
registerDoParallel(3) # 2.3 hr on server
...
foreach(i = (1:nrow(RFE_info)), .packages=c('dplyr', 'randomForestSRC')) %dopar% {
  ...
  nump <- ncol(All_dat_preds_loop)-2    # Number predictors (subtracting responses, NLA12_ID)
  ...
  for(j in 1:max(Lake_folds$folds)){
    ...
    set.seed(73) 
    rf <- rfsrc(D202 ~., data = TrainData, samptype="swr", importance=FALSE, splitrule = "mse", mtry=max(floor(mtry_par*nump), 1), ntree=5000, block.size=NULL, nodesize = 1)
    
    rf_pred_j <- predict(rf, newdata=TestData)
    
    TestData <- TestData %>% mutate(Pred_D202 = rf_pred_j$predicted, 
                                    Fold=j)
...
min_it_mae <- which(RFE_info$D202_MeanCV_mae==min(RFE_info$D202_MeanCV_mae))
best_it_mae <- max(which(RFE_info$D202_MeanCV_mae <= RFE_info$D202_MeanCV_mae[min_it_mae] + RFE_info$D202_SE_CV_mae[min_it_mae]))
# Remaining variables
...
RFE_info  %>% filter(NumVars %in% 1:50) %>% 
  ggplot(aes(x=NumVars, y=D202_MeanCV_mae, label=Predictor)) + 
  geom_point(size=3, aes(col=Subset_MAE)) + 
  geom_line(linewidth=1.2) +
  coord_cartesian(ylim=c(.5,1.1)) +
  geom_hline(yintercept=min(RFE_info$D202_MeanCV_mae), size=1) +
  geom_hline(yintercept=RFE_info$D202_MeanCV_mae[min_it_mae] + RFE_info$D202_SE_CV_mae[min_it_mae], lty=2, size=1) +
...
ggsave(paste0(fig_dir, "/D202_RFE_CV_MAE.png"), width=10, height=6)
...
range(Top_MAE_Mod$D202)
range(Top_MAE_Mod$Pred_D202)
...
Test_Dat <- Test_Dat %>% mutate(Pred_D202 = rf_predict_Test$predicted)
...
# New - almost exactly the same MAE, but slightly worse RMSE and bia
# 0.7190159 0.9234754 0.8528068 0.07323844
...
Iso_Errors <- rbind(Test_Errors_D202[,-3], CV_Errors_D202) 

# Iso_Errors$Dataset <- c("D199_test", "D199_CV", "D200_test", "D200_CV", "D202_test", "D202_CV")
Iso_Errors$Dataset <- c("D202_test", "D202_CV")
...
### CHECK THIS, PROBABLY NEED TO CHNAGE
...
Iso_Errors_Units[1:2,2:4] <- (Iso_Errors_Units[1:2,2:4])*(Iso_stats$D202[2])
...
for(j in 1:max(Lake_folds$folds)){
  ...
  TestData <- TestData %>% mutate(Pred_D202 = mean(TrainData$D202),
                                  Fold=j)
...
CV_Resids <- Top_MAE_Mod %>% dplyr::select(Residual_D202, LON_DD83, LAT_DD83)
...
mantel_test <- mantel(dists, dist_res, permutations=1000)
mantel_test$signif # r=0.09 , p=0.001 There is slight spatial autocorrelation in residuals
