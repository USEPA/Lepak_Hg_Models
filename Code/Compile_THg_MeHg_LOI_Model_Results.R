#' ---
#' title: "Summary of THg, MeHg, and LOI model results"
#' output:
#'   html_document:
#'     df_print: paged
#' ---
...
#' Below, model results are presented for THg responses, MeHg responses, and LOI responses. A table of ranked predictors for each model is at the bottom of the document. 
#'
#' **THg models**
#' =

#' log(THg) model without LOI
#' ===
...
#' log(THg) model with LOI included
#' ===
...
#+ echo=FALSE
MeHg_wTHgLOI_RFE <- read.csv("Model_Output/MeHg_wTHgLOI/rf_RFE_info.csv")
MeHg_wTHgLOI_RFE$Iteration <- 1:nrow(MeHg_wTHgLOI_RFE)
MeHg_wTHgLOI_RFE$NumVars <- rev(MeHg_wTHgLOI_RFE$Iteration)
MeHg_wTHgLOI_RFE <- MeHg_wTHgLOI_RFE %>% arrange(NumVars)
#' log(MeHg) model with THg & LOI included 
#' ===
#' * THg, LOI are top predictors, followed by temperature, precipitation, clay, BFI (ratio of base flow to total flow), conductivity, OM, oxygen, chl-a, developed land cover, depth
#' * GEOS-Chem predictors are not important when THg and LOI are included
#' * Unsure what PctNonAgIntrodManagVegCat and PctImpCat are - look up...
#' * ~ 15 vars
#' * Minimum RMSE: `r round(min(MeHg_wTHgLOI_RFE$OOB_rmse), 3)`
#' * Minimum MAE:  `r round(min(MeHg_wTHgLOI_RFE$OOB_mae), 3)`
#+ echo=FALSE, fig.width=10
MeHg_wTHgLOI_RFE %>% filter(NumVars %in% 1:50) %>% ggplot(aes(x=NumVars, y=OOB_rmse, label=Worst_Var)) + 
  geom_point(size=3) + 
  geom_line(size=1.2) +
  coord_cartesian(ylim=c(.30,.45)) +
  geom_hline(yintercept=min(MeHg_wTHgLOI_RFE$OOB_rmse)) +
  theme_minimal(base_size = 19) +
  scale_x_continuous(breaks=seq(2,50,2)) +
  annotate(geom = "text", x=(1:50), y=.45, label=MeHg_wTHgLOI_RFE$Worst_Var[MeHg_wTHgLOI_RFE$NumVars %in% 1:50], angle=90, hjust=1, size=4) +
  ylab("OOB RMSE") + xlab("Number variables")
...
#+ echo=FALSE
MeHg_noTHgLOI_RFE <- read.csv("Model_Output/MeHg_noTHgLOI/rf_RFE_info.csv")
MeHg_noTHgLOI_RFE$Iteration <- 1:nrow(MeHg_noTHgLOI_RFE)
MeHg_noTHgLOI_RFE$NumVars <- rev(MeHg_noTHgLOI_RFE$Iteration)
MeHg_noTHgLOI_RFE <- MeHg_noTHgLOI_RFE %>% arrange(NumVars)
#' log(MeHg) model WITHOUT THg & LOI included 
#' ===
#' * Without THg & LOI included, pH is top predictor, followed by clay, OM, forest cover, temperature, etc.
#' * GEOS-Chem predictors become important when THg & LOI are removed
#' * Some new predictors also pop up (NO3, d180, zooplankton, etc.)
#' * Worse prediction accuracy w/o THG & LOI and takes more predictors to maintain best performance
#' * ~ 27 vars
#' * Minimum RMSE: `r round(min(MeHg_noTHgLOI_RFE$OOB_rmse), 3)`
#' * Minimum MAE:  `r round(min(MeHg_noTHgLOI_RFE$OOB_mae), 3)`
#+ echo=FALSE, fig.width=10
MeHg_noTHgLOI_RFE %>% filter(NumVars %in% 1:50) %>% ggplot(aes(x=NumVars, y=OOB_rmse, label=Worst_Var)) + 
  geom_point(size=3) + 
  geom_line(size=1.2) +
  coord_cartesian(ylim=c(.36,.5)) +
  geom_hline(yintercept=min(MeHg_noTHgLOI_RFE$OOB_rmse)) +
  theme_minimal(base_size = 19) +
  scale_x_continuous(breaks=seq(2,50,2)) +
  annotate(geom = "text", x=(1:50), y=.5, label=MeHg_noTHgLOI_RFE$Worst_Var[MeHg_noTHgLOI_RFE$NumVars %in% 1:50], angle=90, hjust=1, size=4) +
  ylab("OOB RMSE") + xlab("Number variables")
...
#+ echo=FALSE
MeHgTHg_wLOI_RFE <- read.csv("Model_Output/MeHgTHg_wLOI/rf_RFE_info.csv")
MeHgTHg_wLOI_RFE$Iteration <- 1:nrow(MeHgTHg_wLOI_RFE)
MeHgTHg_wLOI_RFE$NumVars <- rev(MeHgTHg_wLOI_RFE$Iteration)
MeHgTHg_wLOI_RFE <- MeHgTHg_wLOI_RFE %>% arrange(NumVars)
#' log(MeHg/THg) model with LOI included 
#' ===
#' * Top predictor is temperature, followed by a couple GEOS-Chem predictors, elevation, precipitation, chl-a, secchi, runoff, NH4, depth, land use/cover, etc.
#' * GEOS-Chem predictors are more important in predicting MeHg/THg relative to predicting just MeHg
#' * LOI is not very important in predicting MeHg/THg (25th in importance)
#' * ~ 25 vars
#' * Minimum RMSE: `r round(min(MeHgTHg_wLOI_RFE$OOB_rmse), 3)`
#' * Minimum MAE:  `r round(min(MeHgTHg_wLOI_RFE$OOB_mae), 3)`
#+ echo=FALSE, fig.width=10
MeHgTHg_wLOI_RFE %>% filter(NumVars %in% 1:50) %>% ggplot(aes(x=NumVars, y=OOB_rmse, label=Worst_Var)) + 
  geom_point(size=3) + 
  geom_line(size=1.2) +
  coord_cartesian(ylim=c(.32,.5)) +
  geom_hline(yintercept=min(MeHgTHg_wLOI_RFE$OOB_rmse)) +
  theme_minimal(base_size = 19) +
  scale_x_continuous(breaks=seq(2,50,2)) +
  annotate(geom = "text", x=(1:50), y=.5, label=MeHgTHg_wLOI_RFE$Worst_Var[MeHgTHg_wLOI_RFE$NumVars %in% 1:50], angle=90, hjust=1, size=4) +
  ylab("OOB RMSE") + xlab("Number variables")



#+ echo=FALSE
MeHgTHg_noLOI_RFE <- read.csv("Model_Output/MeHgTHg_noLOI/rf_RFE_info.csv")
MeHgTHg_noLOI_RFE$Iteration <- 1:nrow(MeHgTHg_noLOI_RFE)
MeHgTHg_noLOI_RFE$NumVars <- rev(MeHgTHg_noLOI_RFE$Iteration)
MeHgTHg_noLOI_RFE <- MeHgTHg_noLOI_RFE %>% arrange(NumVars)
#' log(MeHg/THg) model WITHOUT LOI included 
#' ===
#' * Very similar results as previous model with LOI included because LOI was not very important in model above
#' * ~ 23 vars
#' * Minimum RMSE: `r round(min(MeHgTHg_noLOI_RFE$OOB_rmse), 3)`
#' * Minimum MAE:  `r round(min(MeHgTHg_noLOI_RFE$OOB_mae), 3)`
#+ echo=FALSE, fig.width=10
MeHgTHg_noLOI_RFE %>% filter(NumVars %in% 1:50) %>% ggplot(aes(x=NumVars, y=OOB_rmse, label=Worst_Var)) + 
  geom_point(size=3) + 
  geom_line(size=1.2) +
  coord_cartesian(ylim=c(.32,.5)) +
  geom_hline(yintercept=min(MeHgTHg_noLOI_RFE$OOB_rmse)) +
  theme_minimal(base_size = 19) +
  scale_x_continuous(breaks=seq(2,50,2)) +
  annotate(geom = "text", x=(1:50), y=.5, label=MeHgTHg_noLOI_RFE$Worst_Var[MeHgTHg_noLOI_RFE$NumVars %in% 1:50], angle=90, hjust=1, size=4) +
  ylab("OOB RMSE") + xlab("Number variables")
...
#' log(LOI) model with GEOS-Chem predictors
#' ===
...
Pred_Table <- data.frame(THg_noLOI=THg_withoutLOI_RFE$Worst_Var[1:50], 
                         ...
                         MeHg_withTHgLOI=MeHg_wTHgLOI_RFE$Worst_Var[1:50],
                         MeHg_noTHgLOI=MeHg_noTHgLOI_RFE$Worst_Var[1:50],
                         
                         MeHgTHgRatio_withLOI=MeHgTHg_wLOI_RFE$Worst_Var[1:50],
                         MeHgTHgRatio_noLOI=MeHgTHg_noLOI_RFE$Worst_Var[1:50],
                         
                         LOI=LOI_RFE$Worst_Var[1:50],
                         LOI_noGEOSChem=LOInoHg_RFE$Worst_Var[1:50])
write.csv(Pred_Table, "Tables/Top50preds_THg_MeHg_LOI_Mods.csv", row.names = FALSE)
...
Print_Table <- data.frame(Rank=1:50,
                          ...
                          MeHg_withTHgLOI=str_trunc(MeHg_wTHgLOI_RFE$Worst_Var[1:50], 20),
                          MeHg_noTHgLOI=str_trunc(MeHg_noTHgLOI_RFE$Worst_Var[1:50], 20),
                          
                          MeHgTHgRatio_withLOI=str_trunc(MeHgTHg_wLOI_RFE$Worst_Var[1:50], 20),
                          MeHgTHgRatio_noLOI=str_trunc(MeHgTHg_noLOI_RFE$Worst_Var[1:50], 20),                         
...
# library(knitr); rmarkdown::render("Code/Compile_THg_MeHg_LOI_Model_Results.R")
