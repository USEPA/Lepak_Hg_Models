...
registerDoParallel(12) # 2.03 hr on laptop
...
foreach(i = (1:nrow(RFE_info)), .packages=c('dplyr', 'randomForest')) %dopar% {
  ...
  for(j in 1:max(Lake_folds$folds)){
    ...
    set.seed(73) 
    rf  <- randomForest(log10LOI ~ ., data=TrainData, 
...
for(i in 1:nrow(RFE_info)){
  ...
  CV_Stats <- All_dat_Pred %>% group_by(Fold) %>% summarize(
    MAE=mean(abs(log10LOI-Pred)), 
    RMSE=sqrt(mean((log10LOI-Pred)^2)), 
    MSE=mean((log10LOI-Pred)^2),
    Bias=mean(Pred-log10LOI), 
    n=n()) 
...
Top_MAE_Mod %>% ggplot(aes(x=log10LOI, y=Pred)) +
  ...
  theme_minimal() +
  coord_cartesian(xlim=c(-2.75,1), ylim=c(-2.75,1))+
  theme(text=element_text(size=20)) +
  xlab("Observed log10LOI") + ylab("Predicted log10LOI")
...
Top_RMSE_Mod %>% ggplot(aes(x=log10LOI, y=Pred)) +
  geom_abline(intercept=0, slope=1, color="black", size=1.1) +
  geom_abline(intercept=1, slope=1, color="black", linetype="dashed", size=1.1) +
  geom_abline(intercept=-1, slope=1, color="black", linetype="dashed", size=1.1) +
  geom_abline(intercept=log10(5), slope=1, color="gray50", linetype="dashed", size=.8) +
...
nump <- ncol(Train_run)-1    # Number predictors (subtracting response)

# Fit final model  
set.seed(73) 
rf.final  <- randomForest(log10LOI ~ ., data=Train_run, 
                          mtry=max(floor(mtry_par*nump), 1), 
                          ntree=5000,
                          nodesize=1,
                          keep.forest=T,
...
Test_Errors <- Test_Dat  %>% summarize(
  ...
  Bias=mean(Pred-log10LOI)) 
#         MAE      RMSE        Bias
#   0.1637548 0.2079033 -0.05457814
...
# Naive test errors
naive_pred <- mean(Train_Dat$log10LOI)

Naive_Test_Errors <- Test_Dat  %>% summarize(
  Naive_MAE=mean(abs(log10LOI-naive_pred)), 
  Naive_RMSE=sqrt(mean((log10LOI-naive_pred)^2))) 
...
for(j in 1:max(Lake_folds$folds)){
  ...
  TestData <- TestData %>% mutate(Pred = mean(TrainData$log10LOI), Fold=j)
...
# Visualize errors 
Test_Dat %>% ggplot(aes(x=log10LOI, y=Pred)) +
  geom_abline(intercept=0, slope=1, color="black", size=1.1) +
  geom_abline(intercept=1, slope=1, color="black", linetype="dashed", size=1.1) +
  geom_abline(intercept=-1, slope=1, color="black", linetype="dashed", size=1.1) +
  geom_abline(intercept=log10(5), slope=1, color="gray50", linetype="dashed", size=.8) +
...
Test_Dat$Residual <- Test_Dat$log10LOI - Test_Dat$Pred
Top_MAE_Mod$Residual <- Top_MAE_Mod$log10LOI - Top_MAE_Mod$Pred
...
mantel_test$signif # r=0.02914 , p=0.035964  Very slight spatial autocorrelation in residuals
...
for(i in 1:length(final.preds)){
  ...
  autoplot(partial1, size=1.2) + theme_minimal() + xlab(paste0(pred_lab)) + ylab("log10LOI") +
    theme(text=element_text(size=20))  #+
