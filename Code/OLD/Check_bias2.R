...
NARS_all <- read_excel("Data/NARS_Hg_isotopes_020521.xlsx") # 2664 - previously 1147 (1072 have Hg data)
...
# Final: Fe2O3Ws (least correlated), Precip09Cat (most recent), logPopDen2010Ws
...
NARS_all_vars_Final <- NARS_all_vars %>% dplyr::select(NLA12_ID, Status, Status2, Status3, LOI_PERCENT, logArea, ELEVATION, logChla, logChloride, logMinOXYGEN, Particle_Hg_HgPConc_ng_m3, WetLossLS_kg_s, EPA_REG)



NARS_all_vars_Final_complete <- NARS_all_vars_Final[complete.cases(NARS_all_vars_Final),] # 969 have all 9

length(unique(NARS_all_vars_Final$NLA12_ID))


Both_all_vars_Final <- left_join(NARS_all_vars_Final, LakeCat_all_vars_Final)

Both_all_vars_Final <- Both_all_vars_Final[complete.cases(Both_all_vars_Final),] # 964 have all 10
...
sum(!is.na(NARS_all_vars_Final$LOI_PERCENT)) # 1582/2664

NARS_all_vars_Final %>% group_by(Status) %>% summarize(sum(is.na(LOI_PERCENT))) # all missing are in rest
...
ks.test(x=Both_all_vars_Final$LOI_PERCENT[Both_all_vars_Final$Status3=="Done"], y=Both_all_vars_Final$LOI_PERCENT[Both_all_vars_Final$Status3=="Ready/Rest"]) # p=.015. # equal CDF

# LOI % same but many missing values
...
sum(is.na(NARS_all_vars_Final$logArea)) # 0/2664
...
ks.test(x=NARS_all_vars_Final$logArea[NARS_all_vars_Final$Status3=="Done"], y=NARS_all_vars_Final$logArea[NARS_all_vars_Final$Status3=="Ready/Rest"]) # p<2.2e-16. # equal CDF

ks.test(x=Both_all_vars_Final$logArea[Both_all_vars_Final$Status3=="Done"], y=Both_all_vars_Final$logArea[Both_all_vars_Final$Status3=="Ready/Rest"]) # p=.76. # equal CDF
...
sum(is.na(NARS_all_vars_Final$ELEVATION)) # 0/2664
...
ks.test(x=NARS_all_vars_Final$ELEVATION[NARS_all_vars_Final$Status3=="Done"], y=NARS_all_vars_Final$ELEVATION[NARS_all_vars_Final$Status3=="Ready/Rest"]) # 3.796e-10

ks.test(x=Both_all_vars_Final$ELEVATION[Both_all_vars_Final$Status3=="Done"], y=Both_all_vars_Final$ELEVATION[Both_all_vars_Final$Status3=="Ready/Rest"]) # p=.02. # equal CDF
...
sum(!is.na(NARS_all_vars_Final$logChla )) # 1534/2664

NARS_all_vars_Final %>% group_by(Status) %>% summarize(sum(is.na(logChla))) # 1 missing in done, 1533 missing in rest
...
ks.test(x=NARS_all_vars_Final$logChla[NARS_all_vars_Final$Status3=="Done"], y=NARS_all_vars_Final$logChla [NARS_all_vars_Final$Status3=="Ready/Rest"]) # 0.1112

ks.test(x=Both_all_vars_Final$logChla[Both_all_vars_Final$Status3=="Done"], y=Both_all_vars_Final$logChla[Both_all_vars_Final$Status3=="Ready/Rest"]) # p=.35. # equal CDF
...
sum(!is.na(NARS_all_vars_Final$logChloride )) # 1626/2664

NARS_all_vars_Final %>% group_by(Status) %>% summarize(sum(is.na(logChloride))) # 1 missing in done, 1625 missing in rest
...
ks.test(x=NARS_all_vars_Final$logChloride[NARS_all_vars_Final$Status3=="Done"], y=NARS_all_vars_Final$logChloride[NARS_all_vars_Final$Status3=="Ready/Rest"]) # 0.1213

ks.test(x=Both_all_vars_Final$logChloride[Both_all_vars_Final$Status3=="Done"], y=Both_all_vars_Final$logChloride[Both_all_vars_Final$Status3=="Ready/Rest"]) # p=.22. # equal CDF
...
sum(!is.na(NARS_all_vars_Final$logMinOXYGEN  )) # 1560/2664

NARS_all_vars_Final %>% group_by(Status) %>% summarize(sum(is.na(logMinOXYGEN))) # 12 missing in done, 3 missing in ready, 1545 missing in rest
...
ks.test(x=NARS_all_vars_Final$logMinOXYGEN[NARS_all_vars_Final$Status3=="Done"], y=NARS_all_vars_Final$logMinOXYGEN[NARS_all_vars_Final$Status3=="Ready/Rest"]) # 0.1296

ks.test(x=Both_all_vars_Final$logMinOXYGEN[Both_all_vars_Final$Status3=="Done"], y=Both_all_vars_Final$logMinOXYGEN[Both_all_vars_Final$Status3=="Ready/Rest"]) # p=.06. # equal CDF
...
sum(!is.na(NARS_all_vars_Final$Particle_Hg_HgPConc_ng_m3  )) # 92/2664

NARS_all_vars_Final %>% group_by(Status) %>% summarize(sum(is.na(Particle_Hg_HgPConc_ng_m3))) # 92 missing in rest
...
ks.test(x=NARS_all_vars_Final$Particle_Hg_HgPConc_ng_m3[NARS_all_vars_Final$Status3=="Done"], y=NARS_all_vars_Final$Particle_Hg_HgPConc_ng_m3[NARS_all_vars_Final$Status3=="Ready/Rest"]) # 0.00068

ks.test(x=Both_all_vars_Final$Particle_Hg_HgPConc_ng_m3[Both_all_vars_Final$Status3=="Done"], y=Both_all_vars_Final$Particle_Hg_HgPConc_ng_m3[Both_all_vars_Final$Status3=="Ready/Rest"]) # p=.02. # equal CDF
...
sum(!is.na(NARS_all_vars_Final$WetLossLS_kg_s  )) # 92/2664

NARS_all_vars_Final %>% group_by(Status) %>% summarize(sum(is.na(WetLossLS_kg_s))) # 92 missing in rest
...
ks.test(x=NARS_all_vars_Final$WetLossLS_kg_s[NARS_all_vars_Final$Status3=="Done"], y=NARS_all_vars_Final$WetLossLS_kg_s[NARS_all_vars_Final$Status3=="Ready/Rest"]) # 4.98e-05

ks.test(x=Both_all_vars_Final$WetLossLS_kg_s[Both_all_vars_Final$Status3=="Done"], y=Both_all_vars_Final$WetLossLS_kg_s[Both_all_vars_Final$Status3=="Ready/Rest"]) # p=.03. # equal CDF
...
####  EPA_REG  ####
sum(is.na(NARS_all_vars_Final$EPA_REG  )) # 0/2664

NARS_all_vars_Final %>% group_by(Status) %>% summarize(sum(is.na(EPA_REG))) # 0 missing

# Done vs. rest
NARS_all_vars_Final %>% filter(!is.na(EPA_REG)) %>% ggplot(aes(Status3)) + geom_bar(aes(fill=EPA_REG), position="fill") 
NARS_all_vars_Final %>% filter(!is.na(EPA_REG)) %>% ggplot(aes(Status)) + geom_bar(aes(fill=EPA_REG), position="fill") 

# Compare proportions in each ecoregion between Done vs. Rest (chi square test of independence)
chisq.test(table(NARS_all_vars_Final$EPA_REG, NARS_all_vars_Final$Status3)) # p-value = 4.575e-11 diff proportions

chisq.test(table(Both_all_vars_Final$EPA_REG, Both_all_vars_Final$Status3)) # p-value = 2.481e-06 diff proportions
...
sum(!is.na(LakeCat_all_vars_Final$Fe2O3Ws  )) # 2/1256
...
ks.test(x=LakeCat_all_vars_Final$Fe2O3Ws[LakeCat_all_vars_Final$Status3=="Done"], y=LakeCat_all_vars_Final$Fe2O3Ws[LakeCat_all_vars_Final$Status3=="Ready/Rest"]) # 0.2386

ks.test(x=Both_all_vars_Final$Fe2O3Ws[Both_all_vars_Final$Status3=="Done"], y=Both_all_vars_Final$Fe2O3Ws[Both_all_vars_Final$Status3=="Ready/Rest"]) # p=.16. # equal CDF
