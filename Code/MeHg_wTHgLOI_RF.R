...
write.csv(grid_search, "Tables/Grid_Search/MeHg_wTHgLOI_grid_srch.csv", row.names = F)
# grid_search <- read.csv("Tables/Grid_Search/MeHg_wTHgLOI_grid_srch.csv")
...
which(RFE_info$OOB_rmse==min(RFE_info$OOB_rmse)) # 106
which(RFE_info$OOB_mae==min(RFE_info$OOB_mae)) # 106

min(RFE_info$OOB_rmse) # 0.3073116
min(RFE_info$OOB_mae) # 0.2362013
...
RFE_info %>% filter(NumVars %in% 1:50) %>% ggplot(aes(x=NumVars, y=OOB_mae, label=Worst_Var)) + 
  ...
  annotate(geom = "text", x=rev(1:50), y=.35, label=RFE_info$Worst_Var[RFE_info$NumVars %in% 1:50], angle=90, hjust=1, size=4) +
...
RFE_info %>% filter(NumVars %in% 1:50) %>% ggplot(aes(x=NumVars, y=OOB_rmse, label=Worst_Var)) + 
  ...
  scale_x_continuous(breaks=seq(2,50,2)) +
  # geom_text(angle=90, nudge_y=.12) +
  annotate(geom = "text", x=rev(1:50), y=.45, label=RFE_info$Worst_Var[RFE_info$NumVars %in% 1:50], angle=90, hjust=1, size=4) +
...
write.csv(RFE_info_write, paste0(output_dir, "MeHg_wTHgLOI_rf_RFE_info.csv"), row.names = FALSE)
