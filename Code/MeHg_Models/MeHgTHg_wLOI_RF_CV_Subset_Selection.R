...
RFE_info  %>% filter(NumVars %in% 1:50) %>% 
  ...
  annotate(geom = "text", x=rev(1:50), y=.4, label=RFE_info$Worst_Var[RFE_info$NumVars %in% 1:50], angle=90, hjust=1, size=4, col=RFE_info$Point_Col[RFE_info$NumVars %in% 1:50]) +
...
RFE_info  %>% filter(NumVars %in% 1:50) %>% 
  ...
  theme_minimal(base_size = 19) +
  scale_x_continuous(breaks=seq(2,50,2)) +
  annotate(geom = "text", x=rev(1:50), y=.5, label=RFE_info$Worst_Var[RFE_info$NumVars %in% 1:50], angle=90, hjust=1, size=4, col=RFE_info$Point_Col_rmse[RFE_info$NumVars %in% 1:50]) +
...
RFE_info  %>% filter(NumVars %in% 1:50) %>% 
  ...
  theme_minimal(base_size = 19) +
  scale_x_continuous(breaks=seq(2,50,2)) +
  annotate(geom = "text", x=rev(1:50), y=.01, label=RFE_info$Worst_Var[RFE_info$NumVars %in% 1:50], angle=90, hjust=1, size=4) +
...
Test_Errors <- Test_Dat  %>% summarize(
  ...
  Bias=mean(Pred-log10MeHgTratio)) 
#         MAE      RMSE        Bias
#   0.280508 0.3497402 -0.0005718359
...
for(i in 1:length(final.preds)){
  ...
  autoplot(partial1, size=1.2) + theme_minimal() + xlab(paste0(final.preds[i])) + ylab("log10MeHgTratio") +
    theme(text=element_text(size=20))  #+
...
rf.2pd <- partial(rf.final, train=Train_run, pred.var = c("Tmean8110Cat", "pct_WetLossConv"), grid.resolution = 20,  parallel=TRUE,  paropts=list(.packages = "randomForest")) # ,
# increase grid.resolution for finer resolution contour plot (but will increase computation time)
saveRDS(rf.2pd, paste0(output_dir, "PDP/Tmean8110Cat_pct_WetLossConv_PDP.rds"))
doParallel::stopImplicitCluster()
# rf.2pd <- readRDS(paste0(output_dir, "PDP/Tmean8110Cat_pct_WetLossConv_PDP.rds"))
...
autoplot(rf.2pd,  contour = T, legend.title = paste0(response_var)) +
  ...
  theme(text=element_text(size=20))
ggsave(paste0(fig_dir, "PDP/Bivariate/Tmean8110Cat_pct_WetLossConv_PDP.png"), width=7, height=5)
...
doParallel::stopImplicitCluster()
# rf.2pd <- readRDS(paste0(output_dir, "PDP/Hg0DryDep_ELEVATION_PDP.rds"))

autoplot(rf.2pd,  contour = T, legend.title = paste0(response_var)) +
  theme_minimal() +
  scale_fill_continuous_diverging(name=paste0(response_var), palette = 'Blue-Red', mid=mean(range(rf.2pd$yhat)), alpha=1, rev=F) +
  theme(text=element_text(size=20)) 
ggsave(paste0(fig_dir, "PDP/Bivariate/Hg0DryDep_ELEVATION_PDP.png"), width=7, height=5)
